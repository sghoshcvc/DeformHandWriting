...
from transformations.image_size import check_size
from transformations.homography_augmentation import HomographyAugmentation
...
class IAMDataset(Dataset):
    ...
    def __init__(self, gw_root_dir, image_extension='.png',
        ...
        gt_file = os.path.join(gw_root_dir, 'queries.gtp')
        words = []
        train_split_ids = []
        test_split_ids = []
        cnt = 0
        ...
        for elem in words:
            ...
            embeddings.append([unigram_dict[ch] for ch in '/'+elem[1]+'*'])
    ...
    def mainLoader(self, partition=None, transforms=HomographyAugmentation()):
        ...
        if partition == 'test':
            ...
            f = open('lex_iam.txt', 'w')
            for l in unique_word_strings:
                f.write(l + '\n')
            f.close()
    ...
    def __getitem__(self, index):
        ...
        #sample = {'img:word_img,embedding:embedding}
