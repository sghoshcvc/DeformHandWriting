...
from dataset.phoc import build_phoc_descriptor, get_unigrams_from_strings, build_char_descriptor
...
class GWDataset(Dataset):
    ...
    def __init__(self, gw_root_dir, image_extension='.png',
        ...
        self.char_string_embeddings = None
        self.length_string_embeddings = None
        ...
        # load the dataset
        img_filenames = sorted([elem for elem in os.listdir(os.path.join(gw_root_dir, 'images'))
                                if elem.endswith(image_extension)])
        words = []
        for img_filename in img_filenames:
            page_id = '.'.join(img_filename.split('.')[:-1])
            doc_img = img_io.imread(os.path.join(gw_root_dir, 'images', img_filename))
            ...
            annotation_lines = open(os.path.join(gw_root_dir, 'documents', annotation_filename), 'r').readlines()
            ...
            for line in annotation_lines:
                ...
                words.append((word_img, transcr.strip(), page_id))
        ...
        for ind, word in enumerate(word_strings):
            x = len(word)
            # print(word)
        ...
        if partition is not None:
            if partition == 'train':
                ...
                self.word_string_embeddings = [string for string, split_id in zip(self.word_embeddings, self.split_ids)
                                               if split_id != self.cv_split_index]
                self.char_string_embeddings = [string for string, split_id in zip(self.char_embeddings, self.split_ids)
                                               if split_id != self.cv_split_index]
                self.length_string_embeddings = [string for string, split_id in zip(self.length_embeddings,
                                                 self.split_ids) if split_id != self.cv_split_index]
            else:
                self.word_list = [word for word, split_id in zip(self.words, self.split_ids)
                                  if split_id == self.cv_split_index]
                self.word_string_embeddings = [string for string, split_id in zip(self.word_embeddings, self.split_ids)
            ...
            self.length_string_embeddings = self.length_embeddings
    ...
    def __getitem__(self, index):
        ...
        embedding_char = torch.from_numpy(np.array(embedding_char))
        ...
        return word_img, embedding_char, len_embed, class_id, is_query
